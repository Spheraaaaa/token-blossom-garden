...
const Profile = () => {
  ...
  const [isTrcWalletModalOpen, setIsTrcWalletModalOpen] = useState(false);
  ...
  const handleGenerateTrcWalletAddress = async (address: string) => {
    try {
      const { error } = await supabase
        .from('profiles')
    ...
    try {
      ...
      if (data && data.length > 0) {
        const mapped = data.map(tx => {
          const dateObj = new Date(tx.created_at);
          const formattedDate = `${dateObj.getDate().toString().padStart(2, '0')}/${(dateObj.getMonth() + 1).toString().padStart(2, '0')}`;
          let formattedFrozenUntil: string | null = null;
          if (tx.frozen_until) {
            const frozenDate = new Date(tx.frozen_until);
            formattedFrozenUntil = `${frozenDate.getDate().toString().padStart(2, '0')}/${(frozenDate.getMonth() + 1).toString().padStart(2, '0')}/${frozenDate.getFullYear()}`;
          }
  ...
  return (
    ...
    >
      ...
      <WalletAddressModal 
        isOpen={isTrcWalletModalOpen} 
        onClose={() => setIsTrcWalletModalOpen(false)} 
        onGenerated={handleGenerateTrcWalletAddress}
        network="trc20"
      />
